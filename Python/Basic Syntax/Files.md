...
Class: Basic Syntax
Created: Sep 23, 2019 12:57 AM
Edited: Oct 12, 2019 1:07 PM
Reviewed: No
...
`file.txt` 파일을 열고 `cool_doc` 이라는 임시 변수에 저장

`cool_doc`에 있는 내용을 읽어 `cool_contents`에 저장
...
`fields` 변수를 이용해 `'name'`, `'userid'`, `'is_admin'`과 같이 형식을 지정해줬다.

`output_writer.writeheader()` 파일의 첫 줄을 `fields`에 입력된 형식으로 입력한다.

`output_writer.writerow(item)` 반복문을 이용해 한줄씩 파일에 입력한다.
