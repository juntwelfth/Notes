...
- `plt.bar()` 메소드는 다양한 카테고리를 비교할 수 있는 bar chart를 만든다.
    1. `x` : x 포지션의 위치
    2. `y` : 각각 바의 높이
  - 대체로 `x`값에는 0, 1, 2, 3 ... 이런식으로 전달하고 `y`값엔 같은 값을 전달한다.

>`x`값을 일일이 다 적어줄 수도 있지만 len을 이용해주면 더 편하다.

```py
heights = [88, 225, 365, 687, 4333,
    10756, 30687, 60190, 90553]
x_values = range(len(heights))

days_in_year = [88, 225, 365, 687,
    4333, 10756, 30687, 60190, 90553]
plt.bar(range(len(days_in_year)), days_in_year)
plt.show()
```

![a](https://i.imgur.com/nTyckFI.png)
...
# x축 틱의 라벨 설정하기
ax.set_xticklabels(['Mercury', 'Venus',
...
ax.set_xticklabels(['Mercury', 'Venus',
    'Earth', 'Mars', 'Jupiter', 'Saturn',
    'Uranus', 'Neptune', 'Pluto'], rotation=30)
```

![a](https://i.imgur.com/wHWnFD6.png)

## Side-By-Side Bars

- 각각의 카테고리끼리 비교하고 싶을 때 Side-By-Side Bar 형태의 그래프

![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/population_bar_chart.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/entertainment.png)
...
![a](https://i.imgur.com/18Z3yNy.png)
...
unit_topics = ['Limits', 'Derivatives', 'Integrals',
    'Diff Eq', 'Applications']
As = [6, 3, 4, 3, 5]
Bs = [8, 12, 8, 9, 10]
Cs = [13, 12, 15, 13, 14]
Ds = [2, 3, 3, 2, 1]
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/variable_error.png)
...
plt.bar(range(len(ounces_of_milk)),
    ounces_of_milk, yerr=error, capsize=5)

plt.show()
```

![a](https://i.imgur.com/oWMkHDy.png)
...
# 에러영역
plt.fill_between(x_values, y_lower,
    y_upper, alpha=0.2)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/fill_between.png)
...
plt.fill_between(months, y_lower,
    y_upper, alpha=0.2)

plt.show()
```

![a](https://i.imgur.com/BIPE0hl.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/budget_skew.png)

- 위 파이차트는 뭔가 이상하고 기울어져있다.
- Matplotlit를 이용해 파이차트를 만들땐 항상 각각 축을 지정해줘야 한다.
- `plt.axis('equal')`

![a](Untitled-81b020a9-d840-48d6-adef-0f80753fa834.png)

```py
from matplotlib import pyplot as plt

payment_method_names = ["Card Swipe", "Cash",
    "Apple Pay", "Other"]
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/budget_chart.png)
...
```py
budget_data = [500, 1000, 750, 300, 100]
budget_categories = ['marketing', 'payroll',
    'engineering', 'design', 'misc']

plt.pie(budget_data)
plt.legend(budget_categories)
```

![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/pie_legend.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/pie_labels.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/budget_chart_full.png)
...
![a](https://i.imgur.com/xs9U7S6.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/histogram.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/histogram_range.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/overlap_hist.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/alpha_histograms.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/step_histogram.png)
...
![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/different_hist.png)

- 이러한 문제를 해결하기 위해 `normed=True` 키워드를 이용해 노멀라이즈 해줄 수 있다.
- `normed=True` 키워드는 각 bin에 속하는 개수를 전체 개수로 나눈 비율을 각 bar의 높이로 사용하게 된다. (뭔말인지)

![a](https://s3.amazonaws.com/codecademy-content/courses/matplotlib/normalized_hist.png)
...
![a](https://s3.amazonaws.com/codecademy-content/programs/dataviz-python/unit-3/pickachart.svg?sanitize=true)
